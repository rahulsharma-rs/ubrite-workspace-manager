...
def ensure_directories(log_initialization=True):
    ...
    # Main root directory
    rc_space_root = current_app.config.get('RC_SPACE_ROOT', os.path.join(os.path.expanduser('~'), 'RCSpace'))
    ...
    config_dir = current_app.config.get('CONFIG_DIR', os.path.join(rc_space_root, 'config'))
    ...
    for directory in [rc_space_root, ubrite_root, db_root, temp_root, logs_dir, cache_dir, uploads_dir, config_dir]:
...
def get_directory_status():
    ...
    return {
        'rc_space_root': os.path.exists(rc_space_root),
